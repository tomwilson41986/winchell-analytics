...
export default function ConversionTab({ records }: { records: SaleRecord[] }) {
  const [year, setYear] = useState('all')
  const [sale, setSale] = useState('all')
  const [gradeAOnly, setGradeAOnly] = useState(false)

  const years = useMemo(() => distinctValues(records, 'year'), [records])
  const sales = useMemo(() => distinctValues(records, 'sale'), [records])
    ...
    <>
      ...
      <section className="section">
        <div className="section__head">
          ...
          <span className="section__note">{intFmt(filtered.length)} records in view</span>
        </div>
        <ChartCard title="Offered → track outcomes" subtitle="Headcount at each stage">
          {has ? (
